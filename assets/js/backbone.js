...
//	==================================================
	...
	function adjustHeaderHeight() {
		var	winWidth	=	$(window).width(),
			winHeight	=	$(window).height()-350;

		$('.home-template #header').css({ 'height': winHeight });

		if (winHeight > 320) {
			$('.home-template #logo-description').css({ 'marginTop': ((winHeight - $('#logo-description').height()) / 2) });
		}
	}
...
//	==================================================
	...
	$(document).ready(function() {
		$("#no-touch-navigation-top .links a:not('.nav-logo-link')").each(function() {
			var	link_URL = $(this).attr('href'),
				link_text = $(this).html();

			$('#touch-navigation-top .links').append('<li class="tm-small"><a href="' + link_URL + '" target="_self">' + link_text + '</a></li>')
		});
	});
...
//	==================================================
	...
	$(document).ready(function() {
		$('#navigation-toggle-top').click(function(e) {
			e.preventDefault();

			$(this).next('#menu-slide-top').slideToggle();
		});
	});
